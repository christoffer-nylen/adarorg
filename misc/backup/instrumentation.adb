...
package body Instrumentation is
   ...
   procedure SetStreams(Out_File : File_Type := Output_File_Handle) is
   begin
   ...
   procedure RestoreStreams is
   begin
      Ada.Wide_Text_IO.Set_Input(Standard_Input);
      Ada.Wide_Text_IO.Set_Output(Standard_Output);
   end RestoreStreams;

   procedure PushInsertionPoint(Line : Text_Line_Number; Column : Text_Column_Number) is
   begin
      Text_Positions.Push((Line => Line, Column => Column), Corresponding_Declarative_Part);
   end PushInsertionPoint;

   procedure PushPredicateExpression(Expr : Asis.Expression) is
   begin
     Marked_Expressions.Push((Expression => Expr, Id => Predicate.Predicate_Counter), Predicate_Expressions);
   end PushPredicateExpression;

   procedure InputFileName(Input : Wide_String) is
      use Ada.Command_Line;
      use Ada.Characters.Handling;
   begin
      File_Name(Input'Range) := Input(Input'Range);
      File_Name_Length := Input'Length;
      Ada.Wide_Text_IO.Open(Input_File_Handle, In_File, To_String(Input) & ADA_BODY_FILE_EXTENSION);
      --TODO: Ta bort argument
      declare
         Output : Wide_String(1..100);
         Output_Length : Natural;
      begin
         Context_Arguments.Get_Unit_Name(Output, Output_Length);
         Ada.Wide_Text_IO.Create(Output_File_Handle, Out_File, To_String(Output(1..Output_Length)) & RORG_INSTRUMENTED_FILE_EXTENSION);
      end;
   exception
      when others =>
   ...
   procedure CopyInputToOutput(Line : Text_Line_Number; Column : Natural := 0) is
   ...
   begin
      ...
      for I in Current_Line_Number..To_Count(Line) loop
         if I=To_Count(Line) and Column /= 0 then
            ...
            begin
               Ada.Wide_Text_IO.Get(Text_Line(1..(Column-1)));
               Ada.Wide_Text_IO.Put(Text_Line(1..(Column-1)));
            end;
         else
            declare
               Text_Line : Wide_String := Ada.Wide_Text_IO.Get_Line;
            begin
               Ada.Wide_Text_IO.Put_Line(Text_Line);
            end;
         end if;
      end loop;
   end CopyInputToOutput;
   ...
   procedure CloseFile is
      procedure CopyUntillEof is
      begin
         while not Ada.Wide_Text_IO.End_Of_File (Input_File_Handle) loop
            declare
               Text_Line : Wide_String := Get_Line(Input_File_Handle);
            begin
               Ada.Wide_Text_IO.Put_Line(Text_Line);
            end;
         end loop;
   ...
   begin
      if Data.Contains_Function_Call then
         return
           CLAUSE_ARG &
           Wide_Character'Val(Character'Pos('A')+Active_Clauses.Number_Of_Clauses-Clause_Index) &
           UNDERSCORE_TOK & Arg;
      else
   ...
   begin
      ...
      if Is_Relational_Clause(Clause_Node.Data.Element) and Op=Asis.Not_An_Operator then
      ...
      procedure Iter(N : Predicate.Predicate_Tree.Node_Access) is
         ...
         begin
            if Abstract_Mode then
               Put(Wide_Character'Val(Character'Pos('A')+Active_Clauses.Number_Of_Clauses-N.Data.Relative_Id));
            else
               --Put("X");
   ...
   procedure Generate_RORG_Mark_Function_Declaration(Predicate_Pairs : Determining_Test_Set.Test_Pairs.Stack) is
      ...
      procedure Generate_Function_Head is
         ...
         Expr : Asis.Expression := Marked_Expressions.Peek(Predicate_Expressions).Expression;
         Expr_Span : Span := Element_Span(Expr);
         Text_Lines : Line_List := Lines(Expr);
         ...
         procedure Generate_Parameters is
            ...
            begin
               ...
               declare
                  Expr : Asis.Definition := Specification_Subtype_Definition(Corresponding_Name_Declaration(Identifier));
               begin
                  case Discrete_Range_Kind(Expr) is
                     ...
                     when A_Discrete_Range_Attribute_Reference =>
                        declare
                           Prefix_Expr : Asis.Expression := Prefix(Range_Attribute(Expr));
                        begin
         ...
         procedure Generate_Clauses is
            ...
            Clause_Node : Predicate.Predicate_Tree.Node_Access;
         begin
            for Clause_Index in reverse 1..Active_Clauses.Number_Of_Clauses loop
               Active_Clauses.Get(Clause_Node, Clause_Index);
               Indentation;
               Put(COMMENT_TOK & SPACE_TOK & CLAUSE_COMMENT & SPACE_TOK);
               Put(Wide_Character'Val(Character'Pos('A')+Active_Clauses.Number_Of_Clauses-Clause_Index));
               ...
               declare
                  Text_Lines2 : Line_List := Lines(Enclosing_Element(Clause_Node.Data.Element));
               begin
      ...
      begin
         ...
         Put_Line(COMMENT_TOK & SPACE_TOK & FUNCTION_NAME_TOK &
                  Trim(Natural'Wide_Image(Predicate.Predicate_Counter),Both));
         Indentation;
      ...
      procedure Generate_Function_Declaration_Part is
         ...
         procedure Generate_Declaration(Clause_Index : Positive; Clause_Node : Predicate.Predicate_Tree.Node_Access; Arg : String_Type) is
            ...
            Data : Predicate.Clause_Information := Clause_Node.Data;
            ...
            begin
               case Expression_Kind(Node.Data.Element) is
                  ...
                  when A_Function_Call =>
                     declare
                        Decl : Asis.Declaration := Corresponding_Called_Function(Node.Data.Element);
                        Result : Asis.Element := Result_Profile(Decl);
                     begin
      ...
      procedure Generate_Function_Body(Index : Positive; Depth : Natural) is
         ...
         procedure Generate_Rorg_Coverage(Depth : Natural) is
            ...
            procedure Generate_Relational_Test_Coverage(Clause_Value : Boolean; Clause_Index : Positive) is
               ...
               Op_Kind : Asis.Operator_Kinds := Operator_Kind(Clause_Node.Data.Element);
         ...
         begin
            ...
            for Clause in reverse 1..Active_Clauses.Number_Of_Clauses loop
               if Is_Determining_Clause(Current_Pair, Clause) then
                  -- RORG CLAUSE COMMENT
                  New_Line;
                  Indentation;
                  Put(COMMENT_TOK & SPACE_TOK & CLAUSE_COMMENT & SPACE_TOK &
                      Wide_Character'Val(Character'Pos('A')+Active_Clauses.Number_Of_Clauses-Clause) &
                      COLON_TOK);
                  New_Line;

                  Active_Clauses.Get(Clause_Node, Clause);
                  Generate_Relational_Test_Coverage(Clause_Boolean_Value(Current_Pair, Clause), Clause);
               end if;
               --if Clause /= Active_Clauses.Number_Of_Clauses then
               --   New_Line;
               --end if;
      ...
      begin
         if Test_Covered(Left_Child(Index)) then
            Indentation;
            Put_Line(IF_NOT_TOK & SPACE_TOK & LEFT_PAREN_TOK & Source(Active_Clauses.Number_Of_Clauses-Depth) & RIGHT_PAREN_TOK & SPACE_TOK & THEN_TOK);
            ...
            Indentation;
            Put_Line(IF_TOK & SPACE_TOK & Source(Active_Clauses.Number_Of_Clauses-Depth) & SPACE_TOK & THEN_TOK);
            Generate_Function_Body(Right_Child(Index), Depth+1);
            Indentation;
            Put_Line(END_IF_TOK & SEMI_COLON_TOK);
         else
   ...
   begin
      SetStreams;
      CopyInputToOutput(Text_Positions.Peek(Corresponding_Declarative_Part).Line);
   ...
   procedure Generate_RORG_Mark_Calls is
      --Line : Text_Line_Number;
      Number_Of_Predicates : Natural := Marked_Expressions.Length(Predicate_Expressions);
      RM_Expr : Rorg_Marked_Expression;
      procedure Generate_RORG_Mark_Call(RM_Expr : Rorg_Marked_Expression) is
         use Asis.Text;
         use Ada.Strings, Ada.Strings.Wide_Fixed;
         Expr_Span : Span := Element_Span(RM_Expr.Expression);
      ...
      begin
         CopyInputToOutput(Expr_Span.First_Line, Expr_Span.First_Column);
